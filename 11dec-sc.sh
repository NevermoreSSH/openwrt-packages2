sed -i "s/DISTRIB_DESCRIPTION=.*/DISTRIB_DESCRIPTION='OpenWrt SNAPSHOT by SolomonRicky (11Dec - SC Fixed V2.2)'/g" /etc/openwrt_release;
...
opkg update;opkg install luci-app-adblock luci-app-ttyd openvpn-openssl luci-app-openvpn luci-app-vnstat luci-app-3ginfo-lite;
...
echo '#Adblock Reload Every Hour' | tee -a /etc/crontabs/root;
echo "0 * * * * /etc/init.d/adblock reload" | tee -a /etc/crontabs/root;
